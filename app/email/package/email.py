...
class OptionOfView(tk.Frame):
    def __init__(self, parent, controller):
        ...
        btn_option3.place(relx=0.5, rely=0.1, anchor=tk.CENTER)
        btn_option4.place(relx=0.5, rely=0.1, anchor=tk.CENTER)
        
        # BACK
        btn_back = tk.Button(self.frame1, text = "BACK", command = lambda: controller.menu(self, client), width=100, fg="maroon", bg="navajowhite")
        btn_back.configure(width = 10)
        btn_back.place(relx = 0.5, rely = 0.35, anchor = tk.CENTER)
    ...
    def show_file_and_status(self, path_folder) :
        ...
        for filename in os.listdir(path_folder) :
            if os.path.isfile(os.path.join(path_folder, filename)) :
                if filename != 'permanent.tmp' :
                    show_name = handle_file.get_file_subject(path_folder + "/" + filename)
                
                    no_mail = handle_file.get_file_name(filename)
                    ...
                    btn_file = tk.Button(self.frame2, text=show_name, command=lambda: self.show_file_content(path_folder, filename), width=100, fg="maroon", bg="navajowhite")
            
        i = int(1)
        for pri in list_btn_file :
