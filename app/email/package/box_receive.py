...
def get_list_mail_update(client : socket, down_mail : int) :
    ...
    if _amount == down_mail :
        ...
        for i in range(1, amount + 1) :
            ...
            email_socket.send_to_server(client, f"RETR {down_mail + 1}")
            email_data = email_socket.get_receive_data(client)
            time.sleep(0.5)
            
            handle_file.decode_file_and_write(original_path + f"email-cache-{down_mail + 1}.txt", email_data)
            time.sleep(1)
            
            down_mail = down_mail + 1